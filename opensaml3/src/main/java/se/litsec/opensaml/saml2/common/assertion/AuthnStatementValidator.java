...
public class AuthnStatementValidator implements StatementValidator {
  ...
  /** {@inheritDoc} */
  @Override
  public final ValidationResult validate(Statement statement, Assertion assertion, ValidationContext context)
   ...
   * @return validation result
   */
  protected ValidationResult validateSsoAndSession(DateTime authnInstant, AuthnStatement statement, Assertion assertion,
    ...
    if (forceAuthn != null && forceAuthn.booleanValue()) {
      if (authnRequestIssueInstant != null) {
        if (authnInstant.getMillis() + AbstractObjectValidator.getAllowedClockSkew(context) < authnRequestIssueInstant.longValue()) {
          String msg = String.format("Invalid Assertion. Force authentication was requested, but authentication "
            ...
            authnInstant, new DateTime(authnRequestIssueInstant, ISOChronology.getInstanceUTC()));
      ...
      if (maxSessionTime != null) {
        if (authnInstant.getMillis() + maxSessionTime.longValue() < AbstractObjectValidator.getReceiveInstant(context)) {
