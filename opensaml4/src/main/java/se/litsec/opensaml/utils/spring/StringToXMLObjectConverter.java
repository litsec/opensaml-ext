...
public abstract class StringToXMLObjectConverter<T extends SAMLObject> implements Converter<String, T> {
   ...
   */
  public StringToXMLObjectConverter(final Class<T> clazz) {
    this.clazz = clazz;
  }
  ...
  public T convert(final String source) {
    T obj = this.clazz.cast(XMLObjectSupport.buildXMLObject(ObjectUtils.getDefaultElementName(this.clazz)));
